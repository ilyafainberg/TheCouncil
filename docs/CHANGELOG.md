...
### Added
- **Command-line mode**: run a debate headlessly, e.g.
  `TheCouncil "Pick a cache strategy" --members ["Claude","Skeptic","contrarian"],["OpenAI","Optimist",""]`.
  Reuses the saved roster when `--members` is omitted, prints only the final decision
  to stdout (`--quiet`), and streams the debate to stderr. CLI mode is AI-only — any
  human members are dropped with a warning. See `--help`.

### Changed
- **Update progress**: the in-app updater now shows a progress dialog (download %,
  MB transferred, then "preparing"/"installing"), so it's clear what it's doing and
  when it's done.
...
[1.0.3]: https://github.com/ilyafainberg/TheCouncil/releases/tag/v1.0.3
